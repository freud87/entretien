  ...
  const supabase = window.supabase.createClient(supabaseUrl, supabaseKey);
   ...
   async function chargerKilometrages() {
      try {
        ...
        // Lire la moyenne depuis l'élément #moyenne
        ...
        // Identifier la dernière intervention par type
        const latestByIntervention = {};
        historiqueData.forEach(item => {
          if (
            !latestByIntervention[item.intervention] ||
            new Date(item.date) > new Date(latestByIntervention[item.intervention].date)
          ) {
            latestByIntervention[item.intervention] = item;
          }
        });

        // Afficher les données avec calculs uniquement pour les dernières interventions
        historiqueData.forEach(item => {
          const row = document.createElement('tr');
          ...
          const isLatest = latestByIntervention[item.intervention]?.id === item.id;
